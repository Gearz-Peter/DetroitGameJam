...
public class KatanaRat : MonoBehaviour
{
    ...
    [SerializeField] GameObject SwordEffectAnim;
    [SerializeField] GameObject RatAfterImagePrefab;
    ...
    IEnumerator SpecialAttackingNumerator(GameObject SelectedCharacter, GameObject enemyObject, Vector3 InitialPos, RatAttackStats stats)
    {
        ...
        int AfterImageTime=15;

        while (Vector2.Distance(SelectedCharacter.transform.position, enemyObject.transform.position) > 250)
        {

            SelectedCharacter.transform.position = Vector3.Lerp(SelectedCharacter.transform.position, enemyObject.transform.position, 4 * Time.deltaTime);
            AfterImageTime--;
            if(AfterImageTime == 0)
            {
                AfterImageTime = 15;
              Instantiate(RatAfterImagePrefab, transform.position, Quaternion.identity, BattleCanvas.transform);
        ...
        Instantiate(SwordEffectAnim, enemyObject.transform.position, Quaternion.identity,BattleCanvas.transform);
